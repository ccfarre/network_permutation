...
for (h in 1:sims){
  ...
  for (i in 1:nrow(res.df)) {
    # Pt 1 - select the Destination data
    ...
    # Pt 2 - allocate additional iterations so the modeled total = observed total
...
# Create output list
null.res.sum.deg <- list(rand.data = res.compiled, obs.interactions = obs.interactions,
                 n.iterations = sims)
...
null.mean <- stats::aggregate(null.res.sum.deg$rand.data[, 3:ncol(null.res.sum.deg$rand.data)],
                              list(null.res.sum.deg$rand.data$Origin), mean)
null.upp <- stats::aggregate(null.res.sum.deg$rand.data[, 3:ncol(null.res.sum.deg$rand.data)],
                             list(null.res.sum.deg$rand.data$Origin), stats::quantile,
                             probs = 1 - p)
null.low <- stats::aggregate(null.res.sum.deg$rand.data[, 3:ncol(null.res.sum.deg$rand.data)],
                             list(null.res.sum.deg$rand.data$Origin), stats::quantile,
                             probs = p)
sd.inters <- stats::aggregate(null.res.sum.deg$rand.data[, 3:ncol(null.res.sum.deg$rand.data)],
                              list(null.res.sum.deg$rand.data$Origin), stats::sd)

# Convert to long table format
obs.inters <- reshape2::melt(null.res.sum.deg$obs.interactions, id.vars = "Origin")
...
output.sum.deg <- merge(obs.inters, null.mean, by.x = c("Origin", "Destination"),
                by.y = c("Origin", "Destination"))
colnames(output.sum.deg )[c(3,4)]<-c("Observed", "Null")
output.sum.deg  <- merge(output.sum.deg , null.low, by.x = c("Origin", "Destination"),
                by.y = c("Origin", "Destination"))
colnames(output.sum.deg )[5] <- paste('Lower.', signif.level * 100, '.CL', sep = "")
output.sum.deg  <- merge(output.sum.deg , null.upp, by.x = c("Origin", "Destination"),
                by.y = c("Origin", "Destination"))
colnames(output.sum.deg )[6] <- paste('Upper.', signif.level * 100, '.CL', sep = "")
output.sum.deg  <- merge(output.sum.deg , sd.inters, by.x = c("Origin", "Destination"),
                by.y = c("Origin", "Destination"))
colnames(output.sum.deg )[7] <- "sd"

# Test significance, comparing observed link strength to the CLs

output.sum.deg $Test <- as.character(rep("Not Significant"))
for (i in 1:nrow(output.sum.deg )) {
  if (output.sum.deg $Observed[i] > output.sum.deg $Upper[i]) {output.sum.deg $Test[i] <- "Above Expected"}
  if (output.sum.deg $Observed[i] < output.sum.deg $Lower[i]) {output.sum.deg $Test[i] <- "Below Expected"}
}

# Calculate the standard effect sizes for observed v. null interactions

output.sum.deg$SES <- with(output.sum.deg , (Observed - Null) / sd)
output.sum.deg$SES[!is.finite(output.sum.deg $SES)] <- NA # Replaces Inf and NaN with NA

# Visualize results

ggplot(output.sum.deg, aes(x = Destination, y = Observed)) +
  geom_errorbar(aes(ymin = Lower.95.CL, ymax = Upper.95.CL), alpha = 0.6) +
  geom_point(size = 2, shape = as.factor(output.sum.deg$Test)) + 
  theme_minimal() +
  ylab("N emails") +
...
output.sum.deg$col <- "red"
output.sum.deg[output.sum.deg$Observed > output.sum.deg$Upper.95.CL , "col"] <- "green"
output.sum.deg[output.sum.deg$Observed < output.sum.deg$Lower.95.CL , "col"] <- "blue"

ggplot(output.sum.deg, aes(x = Destination, y = Observed, colour = col)) +
  geom_errorbar(aes(ymin = Lower.95.CL, ymax = Upper.95.CL), alpha = 0.6) +
...
aggnetwork2 <- graph_from_data_frame(d=output.sum.deg, directed=T) 
clean_net <- subgraph.edges(aggnetwork2, E(aggnetwork2)[Test == "Above Expected" | Test == "Below Expected"])

cbind(
  nodes    = vcount(clean_net),
  nedges  = ecount(clean_net),
  density = edge_density(clean_net),
  recip   = reciprocity(clean_net),
  betweenness  = centr_betw(clean_net)$centralization,
  path_length = mean_distance(clean_net)
)

# calculate the PageRank score for each node
pr2 <- page_rank(clean_net, directed = TRUE, weights = E(clean_net)$Observed)$vector

# identify the most central node
most_central2 <- which.max(pr2)

# print the PageRank score for each node
print(pr2)

# print the most central node
cat("The most central node is:", V(clean_net)[most_central2]$name)
...
# Why is this important? For community detection, among others
